...
onMounted(async () => {
    try {
        ...
        for (const item of foodList.value) {
            const { data: responseRecipe } = await recipeApi.getRecipeByFoodId(item.food.id)
        ...
        >
            ...
            <div class="flex justify-center h-5/6 relative">
                ...
                <div class="w-4/6 p-10 rounded-lg shadow-md border border-gray-300 bg-[#FDFDFD]">
                    ...
                    <div>
                        <ul>
                            <li v-for="item in foodList" :key="item.food.id" class="flex flex-col mb-2">
                                <p class="font-bold">{{ item.food.name }}</p>
                                <ul class="ml-4">
                                    <li v-for="ingredient in recipeList[item.food.id]" :key="ingredient.id">
